    ...
    : is_active_(false)
{
    ...
    // clang-format off
    clear_local_costmap_server = std::make_unique<DummyService<nav2_msgs::srv::ClearEntireCostmap>>(node_, "local_costmap/clear_entirely_local_costmap");
    clear_global_costmap_server = std::make_unique<DummyService<nav2_msgs::srv::ClearEntireCostmap>>(node_, "global_costmap/clear_entirely_global_costmap");
    compute_path_to_pose_server = std::make_unique<ComputePathToPoseActionServer>(node_);
    follow_path_server = std::make_unique<DummyActionServer<nav2_msgs::action::FollowPath>>(node_, "follow_path");
    ...
    // customized
    talker_server = std::make_unique<DummyActionServer<nav2_msgs::action::DummyBehavior>>(node_, "talker");
    // clang-format on
}

ServerHandler::~ServerHandler()
{
    if(is_active_) { deactivate(); }
}

void ServerHandler::activate()
{
    if(is_active_) { throw std::runtime_error("Trying to activate while already activated"); }
...
{
    if(!is_active_) { throw std::runtime_error("Trying to deactivate while already inactive"); }
