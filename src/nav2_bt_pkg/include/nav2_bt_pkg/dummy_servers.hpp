...
template <class ServiceT> class DummyService
{
  public:
    explicit DummyService(const rclcpp::Node::SharedPtr& node, std::string service_name)
        : node_(node),
          ...
          disabled_(false)
    {
        server_ = node->create_service<ServiceT>(service_name, std::bind(&DummyService::handle_service, this, _1, _2, _3));
    }
    ...
    void enable()
    {
        if(disabled_)
        {
            server_ = node_->create_service<ServiceT>(service_name_, std::bind(&DummyService::handle_service, this, _1, _2, _3));
            disabled_ = false;
        }
    }
...
template <class ActionT> class DummyActionServer
{
  public:
  ...
  protected:
    ...
    virtual rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID&,
